...
# validation/mutation/<this>.py -> repo root -> the package under test.
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
SRC = os.path.join(os.environ.get("PKG_BASE", os.path.join(_REPO, "Jul 28")), "nndm")
...
M = [
 ...
 ("flags checkpoint dropped","R/steps/06_flags.R",'  checkpoint(con, "NDMM_FLAGS_ALL")\n\n',''),
 ...
 ("attrition step dropped","R/build_nndm.R",'  list(key = "noother",        flag = "NO_OTHER_CANCER_PRE_LOT1",\n       label = "+ no other cancer in 12-month baseline"),\n',''),
 ...
 ("criteria where drops all but one","R/build_nndm.R","  paste(paste0(alias, setdiff(flags, except), \" = 1\"), collapse = \" AND \")","  paste(paste0(alias, setdiff(flags, except)[1], \" = 1\"), collapse = \" AND \")"),
 ("cohort sql outside the rewrite","R/steps/07_cohort.R","            ON cast(l.PATID as string) = a.PATID","            ON cast(l.PATID as string) = a.PATIDX"),
 ("funnel rows not cumulative","R/steps/07_cohort.R","       WHERE {ndmm_criteria_where(i)}\"))","       WHERE {ndmm_criteria_where(1)}\"))"),
 ("attrition order","R/build_nndm.R",'  list(key = "ce12_fuce",      flag = "CE_lot1_fu",\n       label = "+ CE during follow-up"),\n  list(key = "fuce_nopriortx", flag = "NO_PRIOR_MM_TX",\n       label = "+ no MM oncology therapy in 12-month baseline"),','  list(key = "fuce_nopriortx", flag = "NO_PRIOR_MM_TX",\n       label = "+ no MM oncology therapy in 12-month baseline"),\n  list(key = "ce12_fuce",      flag = "CE_lot1_fu",\n       label = "+ CE during follow-up"),'),
 ("belantamab narrows an earlier row","R/steps/07_cohort.R","  for (i in seq_len(length(NDMM_CRITERIA) - 1L))","  for (i in seq_len(length(NDMM_CRITERIA)))"),
 ...
 ("port deviation unregistered","../../validation/port/nndm.R",'    "AND regexp_replace(trim(code), \'[^A-Za-z0-9]\', \'\') <> \'\'" = 1L)','    "AND regexp_replace(trim(code), \'[^A-Za-z0-9]\', \'\') <> \'\'" = 2L)'),
...
,("readme funnel miscounted","README.md","the nine-step funnel","the ten-step funnel"),
 ...
 ("scope counts not costed","R/steps/00b_lot1_index.R","                                AND {ndmm_criteria_where(except = 'NO_BELANTAMAB', alias = 'f.')}","                                AND {ndmm_criteria_where(2, except = 'NO_BELANTAMAB', alias = 'f.')}"),
 ...
 ("reconcile beyond the cohort","R/steps/00b_lot1_index.R","    INNER JOIN {NDMM_BELANTAMAB_TX} b ON b.PATID = f.PATID","    LEFT JOIN {NDMM_BELANTAMAB_TX} b ON b.PATID = f.PATID"),
 ...
 ("fu ce criterion count only","R/steps/00b_lot1_index.R","                                AND {ndmm_criteria_where(except = 'CE_lot1_fu', alias = 'f.')}\n","                                AND 1 = 1\n"),
 ...
 ("clear run rows before the status","R/build_nndm.R","  write_build_status(con, cfg, \"started\")\n  # Registered the moment the run is marked started","  clear_run_rows(con, cfg)\n  write_build_status(con, cfg, \"started\")\n  # Registered the moment the run is marked started"),
 ...
 ("clear run rows warns instead of stopping","R/build_nndm.R","  if (length(bad))\n    stop(\"Could not clear run \", run_id, \" from:","  if (FALSE)\n    stop(\"Could not clear run \", run_id, \" from:"),
...
TESTS = ["tests/test_runner.R", "tests/test_same_as_overall.R",
         "../../validation/port/nndm.R"]
def stage():
    ...
    # Copied, not linked: one mutation edits the port suite's own registry to
    # check that suite notices, and a symlink would edit the real one.
    shutil.copytree(os.path.join(ROOT, "validation"),
...
for t in ("../../validation/port/nndm.R", "tests/test_same_as_overall.R"):
