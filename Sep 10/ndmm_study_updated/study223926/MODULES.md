...
Rscript tests/run_tests.R                          # 505 checks, no warehouse
```
...
| `tests/run_tests.R` | 505 checks that need no warehouse. The last sections RUN every module for every cohort, parse every statement they emit, and **execute** them against fixtures. |
