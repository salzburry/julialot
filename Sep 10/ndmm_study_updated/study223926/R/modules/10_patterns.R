...
mod_patterns <- function(con, cfg, cohort) {
  ...
  prepare_table(con, wrk("S_TX_ATTRITION"),
    "COHORT string, LOT_NUM int, SOC_CATEGORY string, OUTCOME string,
     N_PATIENTS int, N_DENOM int, PCT double", cohort$key)
  ...
  for (sp in soc_passes(cfg, "e")) {
    run_step(con, paste0("tx_attrition_", cohort$key, "_", sp$key), sprintf("
    INSERT INTO %1$s
    WITH cat AS (
      SELECT e.COHORT, e.LOT_NUM, e.PATID, %3$s AS SOC_CATEGORY,
             CASE
    ...
    SELECT COHORT, LOT_NUM, SOC_CATEGORY, OUTCOME, count(*) AS N_PATIENTS,
           sum(count(*)) OVER (PARTITION BY COHORT, LOT_NUM, SOC_CATEGORY)
             AS N_DENOM,
           round(100.0 * count(*) /
                 sum(count(*)) OVER (PARTITION BY COHORT, LOT_NUM,
                                                  SOC_CATEGORY), 1) AS PCT
    FROM cat GROUP BY COHORT, LOT_NUM, SOC_CATEGORY, OUTCOME",
    wrk("S_TX_ATTRITION"), cohort$key, sp$expr, sp$join),
    qc = sprintf("SELECT count(*) AS n_rows FROM %s WHERE COHORT='%s'",
  ...
  chk <- db_q(con, sprintf(
    "SELECT LOT_NUM, SOC_CATEGORY, sum(N_PATIENTS) AS parts,
            max(N_DENOM) AS whole
     FROM %s WHERE COHORT='%s' GROUP BY LOT_NUM, SOC_CATEGORY",
    wrk("S_TX_ATTRITION"), cohort$key))
  ...
  # And the regimen categories partition the line. This is the claim the whole
  # stratification rests on - a category's rows and the line's own row are the
  # same patients counted two ways - so it is checked here rather than assumed
  # wherever the two are read together.
  if (soc_stratified(cfg)) {
    part <- db_q(con, sprintf(
      "SELECT LOT_NUM, OUTCOME,
              sum(CASE WHEN SOC_CATEGORY = '%3$s' THEN 0 ELSE N_PATIENTS END)
                AS parts,
              max(CASE WHEN SOC_CATEGORY = '%3$s' THEN N_PATIENTS END) AS whole
       FROM %1$s WHERE COHORT='%2$s' GROUP BY LOT_NUM, OUTCOME",
      wrk("S_TX_ATTRITION"), cohort$key, SOC_ALL_CATEGORIES))
    off <- part[!is.na(part$whole) & part$parts != part$whole, , drop = FALSE]
    if (nrow(off))
      stop("PATTERNS ERROR: the regimen categories do not sum to the line for ",
           nrow(off), " outcome(s) of ", cohort$key,
           ", so a category column and the line's own column would disagree. ",
           "Every line the spine carries needs exactly one S_SOC row.",
           call. = FALSE)
  }
}
