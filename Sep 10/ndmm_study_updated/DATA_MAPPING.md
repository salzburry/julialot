...
| `Jul 28/ndmm/R/**`, `Sep 10/lot/engine/R/**` | the SQL actually issued today |
...
| `MED_DIAGNOSIS.DIAG_POSITION` | **zero-padded strings** `01` to `25`, plus `NULL` (1,576,237 rows) — 26 distinct, nothing non-numeric | A string comparison against `'1'` matches nothing; anything reading this must cast, and `try_cast(... as int)` is safe. The test fixture is now zero-padded so a regression fails rather than passing on synthetic data that does not look like the warehouse |
