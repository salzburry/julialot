...
load_category_lookup <- function() {
  ...
  # Tokenise on whitespace / slashes / plus / minus / comma so xlsx
  # rows like "VRd", "K/R/d", "K+R+d", "Bort, Lena, Dexa" all
  # normalise to the same sorted uppercase token vector. Steroid
  # tokens (DEX/DEXA/PRED) are stripped because current
  # LOT_BASE_MEDS excludes steroids (lot_program.R:688 / lot2_5_base.R)
  ...
  list(path           = cat_path,
       ...
       lookup_norm    = setNames(cat_raw,
                                  vapply(reg_raw, function(r) norm_key(r, FALSE),
                                         character(1))),
       lookup_nosterd = setNames(cat_raw,
                                  vapply(reg_raw, norm_key, character(1))))
}

# Match LOT_BASE_MEDS to a category. Tries (in order):
#   1. verbatim string match
#   2. normalised match (strip whitespace/slashes/+/-, uppercase, sort)
#   3. normalised + steroid-stripped match (since current LOT
#      regimens are steroid-free)
# Returns NA when no level matches; the caller surfaces as
# "(uncategorised)".
