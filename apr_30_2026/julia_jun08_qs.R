...
#       1L treatment start). Enforced here: LOT1 LOT_START_DT
#       >= 2017-01-01 (lot_program.R:686) and no belantamab token
#       in LOT_BASE_MEDS at ANY LOT_NUM. NOT enforced (surfaced as
#       explicit limitations in the cohort-def card, not silently
#       dropped): CE >= 12 months before 1L start (pipeline default
#       baseline_days = 183, ~6 months - config_prompts.R:89),
#       CE >= 6 months before MM diagnosis date, CE >= 3 months
#       follow-up, and re-check of adult age at MM-dx.
#
...
# PATIDs; an empty vector means no patient survives the filter.
#
# Verified pipeline semantics (per pipeline_steps.R:356 + lot_program.R:686):
#   - ELIG_COH_FINAL.INDEX_DATE = MM diagnosis qualifying date, NOT
#     the 1L treatment start. Julia's "eligible 1L treatment on/after
#     2017-01-01" therefore filters LOT_LONG.LOT_START_DT at
#     LOT_NUM=1, NOT ELIG_COH_FINAL.INDEX_DATE.
#   - Belantamab exclusion applies to ANY LOT, not just 1L (Julia's
#     PDF says "belantamab in any LOT excluded").
build_ashley_cohort <- function(con, lot_long, final_tbl) {
  ...
  ash <- tryCatch(db_q(con, glue("
    ...
    base AS (
      SELECT cast(PATID as string) AS PATID FROM {final_tbl}
    )
    SELECT l.PATID
    FROM lot1 l
...
load_category_lookup <- function() {
  ...
  # Steroid-strip variant. Current LOT_BASE_MEDS excludes steroids
  # (lot_program.R:688 / lot2_5_base.R), so an xlsx row like
  # "BORT LENA DEXA" would never exact-match LOT_LONG. Build a second
  # lookup keyed on the regimen with DEX/DEXA/PRED stripped, so the
  # caller falls back to that when the verbatim match fails. This is
  # a stopgap until Julia's Q2 (steroid inclusion) lands as a
  # pipeline change.
  strip_steroids <- function(s) {
    tokens <- strsplit(toupper(trimws(s)), "\\s+")[[1]]
    tokens <- tokens[!tokens %in% c("DEX", "DEXA", "DEXAMETHASONE",
                                     "PRED", "PREDNISONE")]
  ...
  list(path           = cat_path,
       ...
       lookup_nosterd = setNames(cat_raw, vapply(reg_raw, strip_steroids,
                                                  character(1))))
}

# Match LOT_BASE_MEDS to a category, trying verbatim first then
# steroid-stripped (since current LOT regimens are steroid-free).
categorise_regimen <- function(reg, cat_data) {
  if (is.na(reg) || reg == "") return(NA_character_)
  if (reg %in% names(cat_data$lookup))
    return(unname(cat_data$lookup[reg]))
  key <- paste(sort(strsplit(toupper(trimws(reg)), "\\s+")[[1]]),
               collapse = " ")
  if (key %in% names(cat_data$lookup_nosterd))
    return(unname(cat_data$lookup_nosterd[key]))
  NA_character_
}
...
build_category_for_cohort <- function(con, lot_long, cohort_def, cat_data) {
  ...
  if (is.null(cat_data)) {
    add_html_card(paste0(
      ...
      '<code>REGIMEN_CATEGORIES_CSV</code> at the file (the env var ',
      'name kept for compatibility; <code>.xlsx</code> is also ',
      'accepted via <code>readxl</code>). Expected headers: one of ',
      '<code>regimen</code> / <code>lot_base_meds</code> / ',
      '<code>med</code> / <code>treatment</code>, plus one of ',
      '<code>category</code> / <code>regimen_category</code> / ',
      '<code>treatment_category</code>. Match falls back to a ',
      'steroid-stripped lookup (DEX/DEXA/PRED tokens removed) since ',
      'current <code>LOT_BASE_MEDS</code> excludes steroids until ',
      'Julia\'s Q2 LOT-rule update lands.</p></div>'),
      section = section,
...
build_cohort_def_card <- function(con, lot_long, final_tbl, ash_ids) {
  ...
  add_html_card(paste0(
    ...
    '; <code>', esc_html(BELA_TOKEN), '</code> token NOT in ',
    '<code>LOT_BASE_MEDS</code> at <b>any</b> LOT_NUM.',
    '</td></tr>',
    '</table>',
