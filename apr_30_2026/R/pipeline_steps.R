...
build_steps <- function(cfg, mat_tables, phases = NULL) {
  ...
  phase_clinical_flags <- function() list(
    ...
    # ----------------------------------------------------------
    # NOTE: Step 18 uses the same 4 therapy sources as LOT pipeline S04 (mma_med_raw):
    ...
    list(
      ...
      sql = glue("
        ...
        UNION ALL
        -- 2) Medical therapy via BILL_PROC_CD (HCPCS/CPT) — aligns with LOT S04 source 2
        SELECT /*+ BROADCAST(c) */
        ...
        WHERE m.FST_DT BETWEEN date('{cfg$study_start}') AND date('{cfg$study_end}')
        UNION ALL
        -- 3) Medical therapy via NDC — aligns with LOT S04 source 3
        SELECT /*+ BROADCAST(c) */
        ...
        WHERE m.FST_DT BETWEEN date('{cfg$study_start}') AND date('{cfg$study_end}')
        UNION ALL
        -- 4) Rx therapy via NDC — aligns with LOT S04 source 4
        SELECT /*+ BROADCAST(c) */
