...
#      LOT_START_TYPE in {SCT_AUTO/SCT_ALLO/SCT_CART/CART/CART_INIT}
#      UNIONed with in-LOT AUTO events from LOT_TX_AUTO_DT_1 and
#      LOT_TX_AUTO_DT_2, deduped, ordered by date. A pure
#      LOT_START_TYPE scan would mis-sequence patients with an AUTO
#      inside (say) LOT1 followed later by a CART line, showing them
#      as just "CART".
#
...
build_sct_cart_sequence <- function(con, lot_long) {
  ...
  # SCT/CART events live in TWO places in LOT_LONG and we must union
  # both, else in-LOT AUTOs are silently dropped:
  #   (a) LOT_START_TYPE in {SCT_AUTO, SCT_ALLO, SCT_CART, CART,
  #       CART_INIT} - the SCT is the LOT-start event itself.
  #   (b) LOT_TX_AUTO_DT_1 / LOT_TX_AUTO_DT_2 - in-LOT AUTO events
  #       inside a non-SCT line of therapy (per lot2_5_base.R). A
  #       LOT_START_TYPE-only scan would miss these and mis-sequence
  #       any patient who had AUTO inside LOT1 then later CART.
  # Falls back to LOT_START_TYPE only if the in-LOT AUTO columns are
  # not in this LOT_LONG (older builds), with a clear log message.
  union_sql <- glue("
    SELECT cast(PATID as string)                    AS PATID,
           LOT_NUM,
  ...
  evt <- tryCatch(db_q(con, union_sql), error = function(e) {
    log_msg("  SCT/CART event union failed (",
            ...
            "LOT_START_TYPE only. Sequences may undercount in-LOT AUTO ",
            "events.")
    db_q(con, glue("
      SELECT cast(PATID as string)                    AS PATID,
             LOT_NUM,
  ...
  add_html_card(paste0(
    ...
    '<p style="color:#555;font-size:13px">Event-based, combining ',
    'LOT-start SCT/CART types and in-LOT AUTO events ',
    '(<code>LOT_TX_AUTO_DT_1</code> / <code>LOT_TX_AUTO_DT_2</code>).</p>',
