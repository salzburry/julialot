...
build_cohort <- function(con, lot_long, final_tbl, map_tbl, enr_tbl,
  ...
  sql <- glue("
    ...
    SELECT l.PATID, l.LOT1_DT, m.MM_DX_DT, d.DEATH_DT,
           -- 3-mo post-LOT1 follow-up status. Per spec, FU is NOT an
           -- exclusion - patients without 3 months FU are retained
           -- with limited follow-up that ends at the censored date of
           -- death. Recorded here as a flag so downstream analyses
           -- can split / censor / report without re-deriving:
           --   'full_3mo'       - strict span covers [LOT1, LOT1+89]
           --   'censored_death' - died before day 90 AND strict span
           --                      covers [LOT1, DEATH_DT]
           --   'limited_fu'     - neither (disenrolled before day 90,
           --                      no death, or gap inside the window)
           CASE
             WHEN EXISTS (SELECT 1 FROM spans_strict s
                          WHERE s.PATID = l.PATID
                            AND s.cov_s <= l.LOT1_DT
                            AND s.cov_e >= date_add(l.LOT1_DT, {POST_LOT_DAYS - 1L}))
               THEN 'full_3mo'
             WHEN d.DEATH_DT IS NOT NULL
              AND d.DEATH_DT <= date_add(l.LOT1_DT, {POST_LOT_DAYS - 1L})
              AND EXISTS (SELECT 1 FROM spans_strict s
                          WHERE s.PATID = l.PATID
                            AND s.cov_s <= l.LOT1_DT
                            AND s.cov_e >= d.DEATH_DT)
               THEN 'censored_death'
             ELSE 'limited_fu'
           END AS FU_STATUS
    FROM lot1 l
    ...
    WHERE l.LOT1_DT >= cast('{ELIGIBLE_1L_FROM}' as date)
      ...
      -- NOTE: 3-mo post-LOT1 FU is NOT an exclusion. Per spec, patients
      -- without 3 months FU are retained with limited follow-up that
      -- ends at the censored date of death. Recorded as FU_STATUS in
      -- the SELECT above; downstream queries can filter on it as
      -- needed (e.g. WHERE FU_STATUS = 'full_3mo' for a sensitivity
      -- analysis, or WHERE FU_STATUS <> 'limited_fu' for the
      -- death-aware retain-all view).
  ")
  db_exec(con, sql)
}

# Cohort summary - total LOT1, final cohort size, and the three FU
# buckets so the card can show the retain-with-limited-FU split.
# The full per-step attrition would require re-running the whole CTE
# chain seven times; not worth the cost when the cohort table already
...
cohort_attrition <- function(con, lot_long) {
  db_q(con, glue("
    SELECT (SELECT count(DISTINCT PATID) FROM {lot_long}
           ...
           (SELECT count(DISTINCT PATID) FROM {wrk(COHORT_TABLE)}
            WHERE FU_STATUS = 'full_3mo')        AS n_full,
           (SELECT count(DISTINCT PATID) FROM {wrk(COHORT_TABLE)}
            WHERE FU_STATUS = 'censored_death')  AS n_death,
           (SELECT count(DISTINCT PATID) FROM {wrk(COHORT_TABLE)}
            WHERE FU_STATUS = 'limited_fu')      AS n_limited"))
}
...
build_cohort_card <- function(att, cl_counts) {
  ...
  criteria <- paste0(
    ...
    '<li>3-mo post-LOT1 FU recorded as <code>FU_STATUS</code> ',
    '(<b>not</b> an exclusion - per spec, patients without full FU ',
    'are retained with limited follow-up that ends at the censored ',
    'date of death).</li>',
  ...
  add_html_card(paste0(
    ...
    '</table>',
    '<p style="color:#555;font-size:13px;margin-top:6px"><b>Follow-up split</b> ',
    '(no patients excluded - retain-with-limited-FU per spec):</p>',
    '<table border="1" cellpadding="6" style="border-collapse:collapse;font-size:13px;margin-bottom:10px">',
    '<tr style="background:#f0f3f5"><th>FU_STATUS</th><th>n patients</th><th>What it means</th></tr>',
    sprintf('<tr><td><code>full_3mo</code></td><td>%s</td><td>strict span covers [LOT1, LOT1 + %s d]</td></tr>',
            format(num(att$n_full[1]),    big.mark = ","), POST_LOT_DAYS - 1L),
    sprintf('<tr><td><code>censored_death</code></td><td>%s</td><td>died before day %s; strict span covers [LOT1, DEATH_DT]</td></tr>',
            format(num(att$n_death[1]),   big.mark = ","), POST_LOT_DAYS),
    sprintf('<tr><td><code>limited_fu</code></td><td>%s</td><td>disenrolled before day %s without death (retained, censor at last enrolment)</td></tr>',
            format(num(att$n_limited[1]), big.mark = ","), POST_LOT_DAYS),
    '</table>',
