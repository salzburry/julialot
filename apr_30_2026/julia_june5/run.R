...
# Attrition counts - cheap re-evaluation of each criterion against the
# delivered LOT1 population so the cohort card can show how each
# filter shrinks the count.
cohort_attrition <- function(con, lot_long, final_tbl, map_tbl, enr_tbl,
                             have_map) {
  bela_map_branch <- if (have_map) glue("
        UNION
        SELECT DISTINCT cast(PATID as string) AS PATID
...
build_cohort_card <- function(att, n_total_lot1) {
  num <- function(x) suppressWarnings(as.numeric(x))
  rows <- list(
    c("LOT1 total in pipeline output",   att$n_lot1[1]),
    c(paste0("+ 1L start &ge; ", ELIGIBLE_1L_FROM), att$n_dt[1]),
    c("+ no belantamab anywhere",        att$n_nobela[1]),
    c(paste0("+ CE &ge; ", PRE_LOT_DAYS, " d pre-LOT1 (gap &le; ", GAP_DAYS, " d)"),
      att$n_ce_lot1[1]),
    c(paste0("+ CE &ge; ", PRE_MM_DAYS, " d pre-MM-dx (gap &le; ", GAP_DAYS, " d)"),
      att$n_ce_mmdx[1]),
    c(paste0("+ CE &ge; ", POST_LOT_DAYS, " d post-LOT1 FU (STRICT, no gap)"),
      att$n_final[1])
  )
  tbl <- paste0(
    '<table border="1" cellpadding="6" style="border-collapse:collapse;font-size:13px">',
    '<tr style="background:#f0f3f5"><th>Step</th><th>n patients</th></tr>',
    paste(vapply(rows, function(r) sprintf(
      '<tr><td>%s</td><td>%s</td></tr>', r[1],
      format(num(r[2]), big.mark = ",")), character(1)), collapse = ""),
    '</table>')
  add_html_card(paste0(
    ...
    'sections inner-join to it.</p>', tbl,
    '<p style="color:#555;font-size:13px;margin-top:10px"><b>Honest scope:</b> ',
    'other-malignancy / pregnancy / no-baseline-MM-therapy exclusions ',
    'are inherited from the parent pipeline\'s 183-day-pre-MM-dx ',
    'window (<code>baseline_days = 183</code>). Recomputing those ',
    'against the 12-mo-pre-LOT1 window belongs in a pipeline edit, ',
    'not here. Death-aware semantics for the 3-mo FU are not ',
    'implemented (would need death-dt access not surfaced in ',
    'persisted output).</p></div>'),
    section = "COHORT", title = "Cohort definition + attrition")
}
...
main <- function() {
  ...
  build_cohort(con, lot_long, final_tbl, map_tbl, enr_tbl, have_map)

  log_msg("Computing attrition...")
  att <- cohort_attrition(con, lot_long, final_tbl, map_tbl, enr_tbl, have_map)
  log_msg(sprintf("  LOT1=%s  +1L>=2017=%s  +noBELA=%s  +CE-LOT1=%s  +CE-MMdx=%s  +FU-3mo=%s",
                  att$n_lot1[1],  att$n_dt[1], att$n_nobela[1],
                  att$n_ce_lot1[1], att$n_ce_mmdx[1], att$n_final[1]))

  dashboard_items <<- list()
  build_cohort_card(att, att$n_lot1[1])
