...
| `lot/engine/R/map_prev.R` | The preceding supply episode for each row, built once as a view, and the test a returning agent has to pass. Every boundary query reads this rather than carrying its own copy. |
| `lot/engine/R/continuing_meds.R` | Fills `LOT_CONTINUING_MEDS`: treatment observed inside a line that the returning-agent rule declines to advance on, recorded beside the regimen rather than in it. |
