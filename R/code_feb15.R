...
# UPDATED: Dynamic criteria ordering (criteria order configurable at runtime)
# ============================================================
#
# Validated against:
...
# ============================================================
# INTERACTIVE CRITERIA ORDER SELECTION
# ============================================================
# Allows user to reorder the attrition criteria at runtime.
# Steps 0 (Base MM dx) and 1 (Qualifying dx) are always first.
# The remaining criteria (default Steps 2-10) can be reordered.
# ============================================================
prompt_criteria_order <- function() {
  if (!should_prompt()) {
    cat("Non-interactive mode: using criteria order from CRITERIA_ORDER env var or default\n")
    return(resolve_criteria_order())
  }

  current_order <- resolve_criteria_order()

  cat("\n")
  cat("============================================================\n")
  cat("  CRITERIA APPLICATION ORDER\n")
  cat("============================================================\n")
  cat("\nSteps 0 (Base MM dx) and 1 (Qualifying dx) are always first.\n")
  cat("The remaining criteria can be reordered below.\n\n")
  cat("Current order:\n")
  for (i in seq_along(current_order)) {
    def <- CRITERIA_DEFS[[current_order[i]]]
    enabled <- is_criterion_enabled(current_order[i])
    status <- if (enabled) "[ON] " else "[OFF]"
    cat(sprintf("  %2d. %-30s %s  (%s)\n", i, def$label, status, def$type))
  }

  cat("\nOptions:\n")
  cat("  Enter     = Keep current order\n")
  cat("  r         = Reverse order\n")
  cat("  Numbers   = Custom order (e.g., 3,1,2,4,5,6,7,8,9)\n")
  cat("  IDs       = Custom order by name (e.g., ce_baseline,age,fu_therapy,...)\n")
  cat("\nYour choice: ")
  response <- trimws(readline())

  if (response == "") {
    cat("Keeping current order.\n")
  ...
  if (tolower(response) == "r") {
    ...
    # Parse comma-separated numbers or criterion IDs
    parts <- trimws(strsplit(response, "[,\\s]+")[[1]])

    # Check if they entered numbers or IDs
    if (all(grepl("^\\d+$", parts))) {
      # Numeric indices
      indices <- as.integer(parts)
      if (any(is.na(indices)) || any(indices < 1) || any(indices > length(current_order))) {
        cat("Invalid indices. Keeping current order.\n")
        return(current_order)
      }
      # Append any missing indices at end
      all_indices <- seq_along(current_order)
      missing_indices <- setdiff(all_indices, indices)
      indices <- c(indices, missing_indices)
      new_order <- current_order[indices]
    } else {
      # Criterion IDs
      new_order <- parts
      valid_ids <- names(CRITERIA_DEFS)
      invalid <- setdiff(new_order, valid_ids)
      if (length(invalid) > 0) {
        cat("Unknown criterion IDs: ", paste(invalid, collapse = ", "), "\n")
        cat("Keeping current order.\n")
        return(current_order)
      }
      # Append any missing
      missing <- setdiff(valid_ids, new_order)
      new_order <- c(new_order, missing)
    }
  }

  cat("\nNew order:\n")
  for (i in seq_along(new_order)) {
    def <- CRITERIA_DEFS[[new_order[i]]]
    enabled <- is_criterion_enabled(new_order[i])
    status <- if (enabled) "[ON] " else "[OFF]"
    cat(sprintf("  %2d. %-30s %s  (%s)\n", i, def$label, status, def$type))
  }
  ...
  if (confirm %in% c("n", "no")) {
    return(prompt_criteria_order())  # Recursive call to restart
  }
...
# Parse criteria order from environment variable (if set)
# Format: comma-separated criterion IDs, e.g. "age,ce_baseline,fu_therapy,..."
cfg$criteria_order <- {
  env_order <- Sys.getenv("CRITERIA_ORDER", unset = "")
  if (nzchar(env_order)) trimws(strsplit(env_order, ",")[[1]]) else character(0)
}

# ============================================================
# DYNAMIC CRITERIA REGISTRY
# ============================================================
# Each criterion is defined independently. The ORDER in which they
# are applied in the attrition table (and Step 24 filter) is
# configurable at runtime via CRITERIA_ORDER env var or interactive prompt.
#
# Steps 0 (base MM dx) and 1 (qualifying IP/OP) are always first.
# The remaining criteria (Steps 2-10 in the default PDF order) can be reordered.
#
# Available criterion IDs:
#   age, fu_therapy, no_bl_therapy, ce_baseline, ce_followup,
#   bl_mm_evidence, other_cancer, pregnancy, clintrial
# ============================================================

DEFAULT_CRITERIA_ORDER <- c(
  "age", "fu_therapy", "no_bl_therapy", "ce_baseline", "ce_followup",
  "bl_mm_evidence", "other_cancer", "pregnancy", "clintrial"
)
...
CRITERIA_DEFS <- list(
  age = list(
    ...
    type = "Inclusion",
    description = "Patients >= min_age years of age in the index year",
    config_key = "apply_age_incl"
  ),
  fu_therapy = list(
    id = "fu_therapy",
    label = "FU therapy required",
    type = "Inclusion",
    description = "Evidence of FDA-approved MM oncology therapy in follow-up",
    config_key = "apply_fu_agents_incl"
  ),
  no_bl_therapy = list(
    id = "no_bl_therapy",
    label = "No baseline therapy (excl)",
    type = "Exclusion",
    description = ">=1 MM therapy claim in baseline period (6mos prior)",
    config_key = "apply_no_bl_agents_incl"
  ),
  ce_baseline = list(
    id = "ce_baseline",
    label = "6-mo baseline enrollment",
    type = "Inclusion",
    description = ">=6 months CE with medical+pharmacy before index",
    config_key = "apply_ce_b_incl"
  ),
  ce_followup = list(
    id = "ce_followup",
    label = "1+ day FU enrollment",
    type = "Inclusion",
    description = ">=1 day CE starting on index date",
    config_key = "apply_ce_f_incl"
  ),
  bl_mm_evidence = list(
    id = "bl_mm_evidence",
    label = "BL MM evidence (excl)",
    type = "Exclusion",
    description = ">=1 MM claim (203.0x/C90.0x) in baseline",
    config_key = "apply_baseline_nondx_excl"
  ),
  other_cancer = list(
    id = "other_cancer",
    label = "Other cancer (excl)",
    type = "Exclusion",
    description = "Evidence of another cancer in baseline",
    config_key = "apply_other_malig_excl"
  ),
  pregnancy = list(
    id = "pregnancy",
    label = "Pregnancy (excl)",
    type = "Exclusion",
    description = "Pregnancy or childbirth in baseline or follow-up",
    config_key = "apply_pregnancy_excl"
  ),
  clintrial = list(
    id = "clintrial",
    label = "Clinical trial (excl)",
    type = "Exclusion",
    description = "Clinical trial participation in baseline and follow-up",
    config_key = "apply_clintrial_excl"
  )
)

# Get SQL condition for a criterion (evaluated at call time to pick up cfg values)
get_criterion_sql <- function(crit_id) {
  switch(crit_id,
    "age"             = glue("AGE_INDEX_YR >= {cfg$min_age}"),
    "fu_therapy"      = "MM_FU_agents = 1",
    "no_bl_therapy"   = "MM_bl_agents = 0",
    "ce_baseline"     = "CE_b = 1",
    "ce_followup"     = "CE_f = 1",
    "bl_mm_evidence"  = "MM_baseline_diag = 0",
    "other_cancer"    = "OTHER_MALIGN_FLAG = 0",
    "pregnancy"       = "PREGNANT_FLAG = 0",
    "clintrial"       = "CLINTRIAL_BASELINE = 0 AND CLINTRIAL_FOLLOWUP = 0",
    stop("Unknown criterion ID: ", crit_id)
  )
}

# Check if a criterion is enabled in cfg
is_criterion_enabled <- function(crit_id) {
  def <- CRITERIA_DEFS[[crit_id]]
  if (is.null(def)) stop("Unknown criterion ID: ", crit_id)
  isTRUE(cfg[[def$config_key]])
}

# Resolve the effective criteria order (from cfg, env, or default)
resolve_criteria_order <- function() {
  order <- cfg$criteria_order
  if (is.null(order) || length(order) == 0 || (length(order) == 1 && !nzchar(order[1]))) {
    order <- DEFAULT_CRITERIA_ORDER
  }
  order <- trimws(order)
  # Validate all IDs exist
  valid_ids <- names(CRITERIA_DEFS)
  invalid <- setdiff(order, valid_ids)
  if (length(invalid) > 0) {
    log_msg("WARNING: Unknown criteria IDs in order: ", paste(invalid, collapse = ", "))
    order <- intersect(order, valid_ids)
  }
  # Ensure all criteria are included (append any missing at end)
  missing <- setdiff(valid_ids, order)
  if (length(missing) > 0) {
    log_msg("NOTE: Appending missing criteria to end of order: ", paste(missing, collapse = ", "))
    order <- c(order, missing)
  }
  order
}
...
build_steps <- function() {
  ...
  # ============================================================
  # BUILD COMBINED CRITERIA SQL (dynamic order)
  # ============================================================
  # Uses the dynamic criteria registry and configurable order.
  # Each criterion is applied in the order specified by cfg$criteria_order.
 
  # This ensures the Step 24 filter and attrition table use the same order.
  criteria_order <- resolve_criteria_order()
  criteria_clauses <- c()

  for (crit_id in criteria_order) {
    if (is_criterion_enabled(crit_id)) {
      criteria_clauses <- c(criteria_clauses, paste("AND", get_criterion_sql(crit_id)))
    }
  }
...
main <- function() {
  ...
  # ============================================================
  # PROMPT FOR CRITERIA ORDER (dynamic reordering)
  # ============================================================
  criteria_order <- prompt_criteria_order()
  cfg$criteria_order <<- criteria_order
  ...
  log_msg("CRITERIA ORDER: ", paste(cfg$criteria_order, collapse = " -> "))
  log_msg("=", SEP_59)
  ...
  tryCatch({
    # ----------------------------------------------------------
    # ATTRITION TABLE: Dynamic order with 30/60/90-day cohorts
    # Steps 0 and 1 are always first, then criteria in configured order
    # ----------------------------------------------------------
    criteria_order <- resolve_criteria_order()
    ...
    q1_30 <- DBI::dbGetQuery(con_env$con, glue("SELECT count(DISTINCT PATID) AS n FROM {work_tbl('ELIG_COH_ALLFLAGS')} WHERE {qual_30}"))
    q1_60 <- DBI::dbGetQuery(con_env$con, glue("SELECT count(DISTINCT PATID) AS n FROM {work_tbl('ELIG_COH_ALLFLAGS')} WHERE {qual_60}"))
    q1_90 <- DBI::dbGetQuery(con_env$con, glue("SELECT count(DISTINCT PATID) AS n FROM {work_tbl('ELIG_COH_ALLFLAGS')} WHERE {qual_90}"))
    record_attrition("01_step1_qualifying", "Step 1: Qualifying dx", q1_30$n, q1_60$n, q1_90$n)

    # Dynamic criteria steps (Steps 2+ in configured order)
    cumul_30 <- qual_30
    cumul_60 <- qual_60
    cumul_90 <- qual_90
    ...
    for (i in seq_along(criteria_order)) {
      crit_id <- criteria_order[i]
      def <- CRITERIA_DEFS[[crit_id]]
      cond <- get_criterion_sql(crit_id)
      enabled <- is_criterion_enabled(crit_id)
      step_num <- i + 1  # Steps 0 and 1 are fixed; criteria start at Step 2

      # Always compute count with this criterion applied (even if not enabled)
      test_30 <- paste(cumul_30, "AND", cond)
      test_60 <- paste(cumul_60, "AND", cond)
      test_90 <- paste(cumul_90, "AND", cond)

      n30 <- DBI::dbGetQuery(con_env$con, glue("SELECT count(DISTINCT PATID) AS n FROM {tbl_name} WHERE {test_30}"))$n
      n60 <- DBI::dbGetQuery(con_env$con, glue("SELECT count(DISTINCT PATID) AS n FROM {tbl_name} WHERE {test_60}"))$n
      n90 <- DBI::dbGetQuery(con_env$con, glue("SELECT count(DISTINCT PATID) AS n FROM {tbl_name} WHERE {test_90}"))$n

      suffix <- if (enabled) "" else " [not applied]"
      label <- sprintf("Step %d: %s%s", step_num, def$label, suffix)
      record_attrition(sprintf("%02d_step%d_%s", step_num, step_num, crit_id), label, n30, n60, n90)

      # Only accumulate if criterion is enabled (affects downstream counts)
      if (enabled) {
        cumul_30 <- test_30
        cumul_60 <- test_60
        cumul_90 <- test_90
