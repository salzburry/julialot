...
build_steps <- function() {
  ...
  list(
    ...
    # ----------------------------------------------------------
    # FIXED: Use baseline_days - 1 for consistency (baseline includes index_date)
    # NOTE: Pregnancy check spans baseline + follow-up intentionally for safety
    list(
      name = "20_pregnancy_flag",
      description = "EXCLUSION: Pregnancy flag (baseline + follow-up)",
...
main <- function() {
  ...
  tryCatch({
    ...
    # FIXED: Cumulative exclusions - each step includes all previous exclusion criteria
    step_num <- 8
    excl_clauses <- c()  # Track cumulative exclusion clauses
