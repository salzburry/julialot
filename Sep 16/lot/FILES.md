...
| `lot/qc/` | the checks on a finished run, asked after the fact, two traces of the returning-drug rules on real patients, and an extract of a patient's inputs for replay. Reads only. `run_lot_qc.R`, `trace_foldin.R`, `trace_returns.R`, `extract_patients.R`. |
