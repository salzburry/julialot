...
| `returning_drug_joins_the_regimen` | A folded drug is in the line's regimen, not only its dates | - | 2L reports TWO drugs, C and B, and its medication count is two. B joins the line's run-out chain as well, so the line cannot end before B stops covering - it ends on the LAST of the base set's cover, which is B's only where B outlasts C. A drug the rule says is part of the line has to be part of it in every reading of the line, and the run-out is one of those readings. | derived |
| `returning_drug_same_day_new_agent` | A genuinely new drug on the same day takes preference | - | 3L opens on day 450 on D, and B belongs to it rather than to 2L. 2L ends exactly where it would have ended had B not come back at all: on C's own cover where that runs out first, otherwise on day 449, the day before D opens. These events do not say how far C's supply reaches, so WHICH of the two it is they do not decide - and that is not the claim. The claim is the part they do decide: the return moved the end of 2L nowhere. | derived |
