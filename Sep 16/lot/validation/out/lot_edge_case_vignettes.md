...
| `returning_drug_same_day_new_agent` | A genuinely new drug on the same day takes preference | - | 3L opens on day 450 on D, and B belongs to it rather than to 2L. 2L ends where its OWN cover ran out - wherever C's supply put that, which on these events is before day 450 and is not day 449. It ends exactly where it would have ended had B not come back at all, which is the whole of the claim: the return moved nothing. | derived |
