...
VIGNETTES <- list(
  ...
  list(id = "returning_drug_joins_the_regimen",
       ...
       events = function(p) rbind(
         ...
         ev(450, "MED", "drug B comes back, while 2L is still running")),
       expected = function(p) paste0(
         "2L reports TWO drugs, C and B, and its medication count is two. The ",
         "line also runs to the last day B's supply reaches, because a drug ",
         "the rule says is part of the line has to be part of it in every ",
         "reading of the line."),
  ...
  list(id = "returning_drug_same_day_new_agent",
       ...
       expected = function(p) paste0(
         ...
         "ends the day before, on its own run-out, exactly where it would have ",
         "ended had B not come back at all."),
