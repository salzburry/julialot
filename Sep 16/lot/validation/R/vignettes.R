...
VIGNETTES <- list(
  ...
  list(id = "returning_drug_same_day_new_agent",
       ...
       expected = function(p) paste0(
         ...
         "ends exactly where it would have ended had B not come back at all: ",
         "on C's own cover where that runs out first, otherwise on day 449, ",
         "the day before D opens. These events do not say how far C's supply ",
         "reaches, so WHICH of the two it is they do not decide - and that is ",
         "not the claim. The claim is the part they do decide: the return ",
         "moved the end of 2L nowhere."),
