...
ORDER <- c("check_settings", "pin_output_schema", "pin_prefix",
           ...
           "build_ndmm_mma_codelist", "check_ndmm_mma_code_types", "check_ndc_shape",
...
local({
  ...
  le <- new.env(parent = globalenv())
  sys.source(normalizePath(file.path(ROOT, "R", "db_utils.R")), envir = le)
  ...
  le2 <- new.env(parent = globalenv()); sys.source(normalizePath(file.path(ROOT, "R", "db_utils.R")), envir = le2)
  ...
  writeLines(c(
    sprintf("sys.source(%s, envir = globalenv())", deparse(normalizePath(file.path(ROOT, "R", "db_utils.R")))),
    "start_run_log()",
    "log_msg('started')",
