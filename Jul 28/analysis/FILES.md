...
| `jul20_studyteam_qs.R` | The July-20 set: the DARA+BORT dual-therapy question, and `q3_cart_screen()`, which counts the patients the CAR-T induction rule touches. Its Q3a melphalan screen is off unless `JUL20_Q3A_MELP=TRUE`. That screen reads persisted tables and answers a one-sentence melphalan rule; the melphalan question is answered by three complete runs in `exploration/melphalan/`. |
