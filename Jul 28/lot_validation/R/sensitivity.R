...
SENS_AXES <- list(
  list(param = "MAP_DISCON_GAP_DAYS", cfg = "map_discon_gap_days",
       ...
       expect = list(n_lines = "none", pct_reaching_lot2 = "none",
                     median_lot1_length = "none", n_patients = "none"),
       confidence = "derived",
       why = paste0("This parameter only labels MAP_DISCON_FLG, a descriptive ",
                    "flag nothing in the line build reads. A line's run-out is ",
                    "the day-supply arithmetic; the spec's line-level 90-day ",
                    "confirmation was dropped from the end-date derivation. So ",
                    "every metric should sit still, and this cell is the ",
                    "sweep's negative control: movement here means a build ",
                    "difference that is not the parameter.")),
