...
M = [
 ...
 ("adjacent groups not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_mm_adjacent_groups(con, cfg)\n","  "),
 ...
 ("belantamab scope counts dropped","R/build_nndm.R","  if (review_tables_on()) build_ndmm_belantamab_scope_counts(con, cfg)\n","  "),
 ...
 ("index agents not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_index_agents(con, cfg)\n","  "),
 ...
 ("checkpoint before its view","R/build_nndm.R",'  build_ndmm_lot1_index(con, cdm_src(cfg$tbl_medical), cdm_src(cfg$tbl_rx))','  checkpoint(con, "NDMM_LOT1_STARTS")\n  build_ndmm_lot1_index(con, cdm_src(cfg$tbl_medical), cdm_src(cfg$tbl_rx))'),
 ...
 ("outputs list","R/build_nndm.R","DELIVERABLES <- c(CORE_DELIVERABLES, REVIEW_TABLES)","DELIVERABLES <- CORE_DELIVERABLES"),
 ...
 ("readme port count stale","README.md","24 added lines","23 added lines"),
 ("readme port total stale","README.md","43 places","42 places"),
...
,("readme funnel miscounted","README.md","the nine-step funnel","the ten-step funnel"),
 ...
 ("reconcile not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_belantamab_reconcile(con, cfg)\n","  "),
 ("reconcile drops the date","R/steps/00b_lot1_index.R","           datediff(b.bel_dt, l1.LOT1_START_DT) AS DAYS_FROM_INDEX","           0                                    AS DAYS_FROM_INDEX"),
 ...
 ("grain table not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_other_malig_grain(con, cfg)\n","  "),
 ...
 ("groups table not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_other_malig_groups(con, cfg)\n","  "),
 ("death not reclamped for CE","R/steps/06_flags.R","             CASE WHEN ec.DEATH_DT IS NULL THEN NULL\n                  ELSE greatest(cast(ec.DEATH_DT as date), l1.LOT1_START_DT)\n             END                           AS DEATH_DT","             cast(ec.DEATH_DT as date)     AS DEATH_DT"),
 ("fu ce counts skip the clamp","R/steps/00b_lot1_index.R","             CASE WHEN b.DEATH_DT IS NULL THEN NULL\n                  ELSE greatest(b.DEATH_DT, l1.LOT1_START_DT) END AS DEATH_DT","             b.DEATH_DT AS DEATH_DT"),
 ("same-day follow-up rejected","R/build_nndm.R","\"sum(CASE WHEN FU_DAYS < 0 THEN 1 ELSE 0 END) AS n_nofu \",","\"sum(CASE WHEN FU_DAYS < 1 THEN 1 ELSE 0 END) AS n_nofu \","),
 ("other cancer inpatient by max","R/steps/04_other_malig.R","             CASE WHEN h.line_inpatient = 1 OR cf.CONF_ID IS NOT NULL","             CASE WHEN h.POS IN ('21','51','61') OR cf.CONF_ID IS NOT NULL"),
 ("inpatient not flagged per line","R/steps/04_other_malig.R","           max(CASE WHEN POS IN ('21', '51', '61')","           max(CASE WHEN FALSE"),
 ("override rows unchecked","R/build_nndm.R","      WHERE o.dx IS NULL\"))$n)","      WHERE 1 = 0\"))$n)"),
 ("mm dx override not guarded","R/build_nndm.R","      WHERE ovr.override = 0\"))$n)","      WHERE 1 = 0\"))$n)"),
 ("primary group rows unchecked","R/build_nndm.R","      WHERE o.tumor_group IS NULL\"))$n)","      WHERE 1 = 0\"))$n)"),
 ("decision files unchecked","R/build_nndm.R","  check_decision_files(con, cfg)\n","  "),
 ("mma codelist unchecked","R/build_nndm.R","  check_mma_codelist(con, cfg)\n","  "),
 ("blank agent accepted","R/build_nndm.R","  if (isTRUE(q$n_noabbr > 0))","  if (FALSE)"),
 ("shared code accepted","R/build_nndm.R","  if (nrow(d) > 0)\n    stop(nrow(d), \"+ code(s) on cl_mma_codelist.csv","  if (FALSE)\n    stop(nrow(d), \"+ code(s) on cl_mma_codelist.csv"),
 ("index tx checkpointed too late","R/steps/00b_lot1_index.R","  checkpoint(con, \"NDMM_INDEX_TX\")\n  db_exec(con, glue(\"\n    CREATE OR REPLACE TEMPORARY VIEW {NDMM_LOT1_STARTS} AS","  db_exec(con, glue(\"\n    CREATE OR REPLACE TEMPORARY VIEW {NDMM_LOT1_STARTS} AS"),
 ("belantamab tx checkpointed too late","R/steps/00b_lot1_index.R","  checkpoint(con, \"NDMM_BELANTAMAB_TX\")\n  scope <- switch(NDMM_BELANTAMAB_SCOPE,","  scope <- switch(NDMM_BELANTAMAB_SCOPE,"),
 ("review tables always on","R/build_nndm.R",'review_tables_on <- function()\n  identical(toupper(Sys.getenv("NDMM_REVIEW_TABLES", unset = "")), "TRUE")','review_tables_on <- function() TRUE'),
 ("review table ungated","R/build_nndm.R","  if (review_tables_on()) build_ndmm_fu_ce_counts(con, cfg)","  build_ndmm_fu_ce_counts(con, cfg)"),
 ("cohort behind the review flag","R/build_nndm.R","  build_ndmm_cohort_table(con, cfg)","  if (review_tables_on()) build_ndmm_cohort_table(con, cfg)"),
 ("decisions banner gated too","R/build_nndm.R","  report_pending_decisions(cfg)","  if (review_tables_on()) report_pending_decisions(cfg)"),
 ("tables off not said","R/build_nndm.R",'  if (!review_tables_on())\n    log_msg("The tables named above were NOT built:','  if (FALSE)\n    log_msg("The tables named above were NOT built:'),
 ("core list widened","R/build_nndm.R",'CORE_DELIVERABLES <- c("NDMM_COHORT", "NDMM_ATTRITION", "NDMM_FLAGS_ALL",','CORE_DELIVERABLES <- c("NDMM_INDEX_AGENTS", "NDMM_COHORT", "NDMM_ATTRITION", "NDMM_FLAGS_ALL",'),
 ("decisions never reported","R/build_nndm.R","  report_pending_decisions(cfg)\n","  "),
 ("decisions gate off","R/build_nndm.R",'  if (identical(toupper(Sys.getenv("NDMM_REQUIRE_DECISIONS", unset = "")), "TRUE") &&\n      length(p))','  if (FALSE)'),
 ("decisions gate blocks for ever","R/build_nndm.R","  p <- pending_decisions(cfg, blocking_only = TRUE)","  p <- pending_decisions(cfg)"),
 ("decisions miss the empty file","R/build_nndm.R","    max(0L, length(readLines(p, warn = FALSE)) - 1L)","    1L"),
 ("decisions miss the fu ce override","R/build_nndm.R","  if (!identical(as.integer(cfg$fu_ce_days), PROTOCOL_FU_CE_DAYS))","  if (FALSE)"),
 ("decisions not recorded","R/build_nndm.R","                       DECISIONS_PENDING = \"STRING\",\n","                       "),
 ("clear failure only warns","R/build_nndm.R",'      stop("Could not clear ", tbl, " of run ", run_id, ": ", err,','      log_msg("Could not clear ", tbl, " of run ", run_id, ": ", err,'),
 ("reconcile one-sided again","R/steps/00b_lot1_index.R","    FROM {NDMM_LOT1_STARTS} l1\n    INNER JOIN {NDMM_BELANTAMAB_TX} b\n            ON cast(b.PATID as string) = cast(l1.PATID as string)\n    LEFT JOIN {wrk('NDMM_COHORT')} c","    FROM {NDMM_LOT1_STARTS} l1\n    INNER JOIN {NDMM_BELANTAMAB_TX} b\n            ON cast(b.PATID as string) = cast(l1.PATID as string)\n    INNER JOIN {wrk('NDMM_COHORT')} c"),
 ("reconcile hides which way","R/steps/00b_lot1_index.R","           CASE WHEN c.PATID IS NULL THEN 0 ELSE 1 END AS IN_COHORT","           1 AS IN_COHORT"),
 ("fu ce counts not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_fu_ce_counts(con, cfg)\n","  "),
 ...
 ("adjacent codes not written","R/build_nndm.R","  if (review_tables_on()) build_ndmm_mm_adjacent_codes(con, cfg)\n","  "),
