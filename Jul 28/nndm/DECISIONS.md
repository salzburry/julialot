...
`MED_ABBR` on treatment episodes in `map_stacked`, not the built lines, which is
what keeps "any LOT" literal: reading `LOT_BASE_MEDS` would bound the question
by `MAX_LOT` and by position within a line.
