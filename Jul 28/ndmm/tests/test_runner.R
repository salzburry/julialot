...
cfg <- list(final_table_name = "NDMM_COH_FINAL")
ok(is_checkpoint("mm_qualifying", "*", cfg), "'*' materializes an ordinary step")
ok(!is_checkpoint("NDMM_COH_FINAL", "*", cfg),
...
Sys.setenv(CHECKPOINT_STEPS = "*")
good <- list(final_table_name = "NDMM_COH_FINAL", object_prefix = "ndmm_",
             persist_to_schema = TRUE)
runs(check_output_contract(good, "NDMM_COH_FINAL", "ndmm_"), "accepts the declared contract")
stops(check_output_contract(modifyList(good, list(final_table_name = "ELIG_COH_FINAL")),
                            "NDMM_COH_FINAL", "ndmm_"),
      "rejects an ambient FINAL_TABLE_NAME")
stops(check_output_contract(modifyList(good, list(object_prefix = "overall_")),
                            "NDMM_COH_FINAL", "ndmm_"),
...
stops(check_output_contract(good, "NDMM_COH_FINAL", "ndmm_"),
...
cat("\n-- pin_output_schema --\n")
clear()
Sys.setenv(DOMINO_USER_NAME = "osk02156", OBJECT_PREFIX = "ndmm_")
...
ok(identical(p$object_prefix, "ndmm_"), "prefix carried onto cfg")
clear()
