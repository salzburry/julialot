...
LOT_QC_CHECKS <- list(
  ...
  list(id = "B8", group = "End reason", severity = "fail",
       what = "discontinuations inside the confirmation window, which should now be none",
       why = paste0("The spec's LOT1_BASE tab carries a rule nulling a run-out ",
                    "within LOT_DISCON_CONFIRM_DAYS of observation end - not ",
                    "enough follow-up to confirm the patient truly ",
                    "discontinued - while its later end-date tabs re-derive the ",
                    "date with no such condition. The study team adjudicated in ",
                    "favour of the tab that has it, because in a real-world ",
                    "claims study a patient we stop seeing fills for has not ",
                    "necessarily discontinued. The build applies it, so this ",
                    "count is now an invariant rather than a measurement: any ",
                    "row here is a line called DISCONTINUATION that the buffer ",
                    "should have censored."),
       needs = c("final", "cohort"),
       sql = function(t, p) counted(paste0("
    SELECT ", mask("l.PATID"), " AS pid, l.LOT_NUM,
