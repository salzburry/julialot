...
`lot/questions/melphalan_lot_rule.md` and measured by `lot/validation`'s
...
`lot/engine/R/melp_rule.R`. It has to be inside `lot/engine`: that package
builds the lines and reads nothing from a sibling, and the rule needs each
line's own induction window - which only exists while that line is being built.
...
Three of the five open questions in `lot/questions/melphalan_lot_rule.md` are
