...
`lot/` is the lines-of-therapy product: the engine that builds the lines and the
checks that sign a run off. This folder carries two documents and no more.
`LOT_RULES.md` is the rules the build applies, one at a time, each naming the
machine-checked vignette that tests it. This file says what is here and what
each file does.
...
The twin of the rules in `LOT_RULES.md`. Every rule there carries a
