...
| `lot/qc/` | thirty-four checks on a finished run, asked after the fact. Reads only. `run_lot_qc.R`. |
...
| path | what it does |
|---|---|
| `run_lot_qc.R` | Runs thirty-four checks against a finished run and refuses one whose own build did not complete. Reads only; writes a report to `out/`. Exit status is 0 when nothing failed and 1 when something did, so it can gate a handover. |
