...
| `R/foldin_rule.R` | The MAP fold-in. A drug from an earlier line coming back joins the line it returns in, when exactly ONE agent advanced the line between that drug's two doses; two or more and the return starts a line (`LOT_RULES.md` 4.8). It counts AGENTS, so one drug opening two lines is one advance and a transplant-opened line is none. `APPLY_MAP_FOLDIN=FALSE` builds without it, as a comparison — `exploration/lot/run_foldin_cells.R`. |
