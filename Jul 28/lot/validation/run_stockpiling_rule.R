...
report_rule <- function(sc) {
  ...
  cat("\nWrites <prefix>STOCKPILE_AGENTS, <prefix>STOCKPILE_IMPACT,\n",
      "<prefix>STOCKPILE_BY_LOT and <prefix>STOCKPILE_BY_MED. It writes no LOT\n",
      "table and rebuilds nothing.\n", sep = "")
}
...
main <- function() {
  ...
  db_exec(con, glue("CREATE OR REPLACE TABLE {ag} AS {
    stock_agents_sql(lines, maps, sc, run_id)}"))
  ...
  cat("\nBy line:\n")
  cat(sprintf("  %-5s %9s %9s %10s %9s %9s %9s\n",
              "LOT", "lines", "affected", "% affected", "patients", "extend", "-MED_ADD"))
