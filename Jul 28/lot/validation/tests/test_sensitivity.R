...
#
#   Rscript "lot/validation/tests/test_sensitivity.R"
...
sys.source(file.path(PARENT, "engine", "R", "load_inputs.R"), envir = e)
e$load_pipeline_inputs(file.path(PARENT, "engine"), "config.csv")
sys.source(file.path(PARENT, "engine", "R", "config_lot.R"), envir = e)
...
cl <- readLines(file.path(PARENT, "engine", "config.csv"), warn = FALSE)
...
sct <- readLines(file.path(PARENT, "engine", "R", "steps", "05_sct.R"), warn = FALSE)
...
bl <- readLines(file.path(PARENT, "engine", "R", "build_lot.R"), warn = FALSE)
contract_keys <- names(get("CONTRACT", envir = local({
  e <- new.env(parent = globalenv()); sys.source(
    file.path(PARENT, "engine", "R", "build_lot.R"), envir = e, keep.source = FALSE); e
})))
