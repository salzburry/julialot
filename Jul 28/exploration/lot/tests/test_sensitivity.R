...
#
#   Rscript "exploration/lot/tests/test_sensitivity.R"
...
sys.source(file.path(LOT, "R", "load_inputs.R"), envir = e)
e$load_pipeline_inputs(file.path(LOT), "config.csv")
sys.source(file.path(LOT, "R", "config_lot.R"), envir = e)
...
cl <- readLines(file.path(LOT, "config.csv"), warn = FALSE)
...
lc <- readLines(file.path(LOT, "R", "line_criteria.R"), warn = FALSE)
...
bl <- readLines(file.path(LOT, "R", "build_lot.R"), warn = FALSE)
contract_keys <- names(get("CONTRACT", envir = local({
  e <- new.env(parent = globalenv()); sys.source(
    file.path(LOT, "R", "build_lot.R"), envir = e, keep.source = FALSE); e
})))
...
# The readers that must refuse a deviating cell now live in their own areas
# beside lot/, so they are reached through the study folder rather than as
# siblings. Naming them here is the point of the check: a cell is only safe to
# build because everything downstream turns one away.
STUDY <- dirname(PARENT)
for (f in c(file.path(ROOT, "R", "run_binding.R"),
            file.path(STUDY, "analysis", "questions", "_setup.R"),
            file.path(STUDY, "reporting", "dashboard", "R", "db_utils_dash.R")))
