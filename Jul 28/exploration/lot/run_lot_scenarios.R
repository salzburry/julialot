...
OPEN_QUESTIONS <- data.frame(
  ...
  WHAT_THE_CODE_DOES_TODAY = c(
    ...
    paste0("The returning agent ends the line as an added medication and can ",
           "start the next line - on a same-day tie with a transplant or ",
           "another drug it may not be the sole starter. The join reading is ",
           "built as the gated APPLY_MAP_FOLDIN mode, off in CONTRACT; ",
           "exploration/lot/run_foldin_cells.R builds and differences it."),
  ...
  SEE_SCENARIO = c("S09", "S05 and S06", "S15",
                   ...
                   paste0("S10 - the drug has run out well before both transplants and ",
                          "the pair holds; S11 - a drug started between them breaks it")),
