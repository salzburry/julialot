...
LOT_SCENARIOS <- list(
  ...
  list(id = "S01", group = "A drug that comes back",
       ...
       story = paste0("A patient starts on daratumumab. Ten days before it runs out they ",
                      ...
                      "they go back on it. "),
       outcome = paste0("Two lines. Lenalidomide counts as a new drug, so it closes the ",
                        "first line and opens the second. The daratumumab coming back ",
                        "does NOT open a third: one drug advanced the line while it was ",
                        "away, so it joins the second line instead. "),
       ...
       lines = c("LOT1  d0-d89   MED  MED_ADD          regimen DARA",
                 "LOT2  d90-d250  MED  DISCONTINUATION  regimen DARA LEN"),
       rule = "LOT_RULES.md 4.1, 4.3, 4.8, 7.4",
       note = paste0("Two lines. LEN starts outside LOT1's 60-day window, so it is an ",
                     "added medication and ends LOT1. DARA's return would have ended ",
                     "LOT2 the same way, and did until the study adopted the fold-in: ",
                     "exactly one drug advanced the line between DARA's two doses, so ",
                     "the return joins LOT2 - its span carries it to d250, and it ",
                     "joins the regimen string, so LOT2 reads DARA LEN."),
  ...
  list(id = "S02", group = "A drug that comes back",
       ...
       outcome = paste0("The same two lines as S01. The longer gap changes nothing: ",
                        "what decides the return is how many drugs advanced the line ",
                        "while it was away, not how long it was away. "),
       ...
       lines = c("LOT1  d0-d89   MED  MED_ADD          regimen DARA",
                 "LOT2  d90-d250  MED  DISCONTINUATION  regimen DARA LEN"),
       rule = "LOT_RULES.md 4.1, 4.3, 4.8, 7.4",
       note = paste0("THE SAME TWO LINES as S01, and that is the point: the gap grew ",
                     "by a month and the answer did not move. The fold-in counts ",
                     "advances between the two doses, not days. The 90-day ",
                     "discontinuation threshold decides nothing here either - S05 and ",
                     "S06 are where it does decide the answer."),
       sql = NULL,
  ...
  list(id = "S06", group = "A drug that comes back",
       ...
       story = paste0("The same patient goes back on it one day later - 90 days after ",
                      "stopping. "),
       outcome = paste0("ONE line, running over the break. Going back on a drug the ",
                        "patient has already had is not new treatment, whatever the ",
                        "gap, so it does not start a line. The line the drug left ",
                        "runs on over the break and ends where the second course ",
                        "ends. "),
       ...
       lines = c("LOT1  d0-d400  MED  DISCONTINUATION  regimen LEN"),
       rule = "LOT_RULES.md 4.3, 5.2",
       note = paste0("ONE line. A later line opens on an agent that was not in ",
                     "the previous regimen, and a drug the patient has had ",
                     "before is not one - so the gap no longer releases it. The ",
                     "line runs over the break instead, which is the other half ",
                     "of the same rule: refuse the restart a line and carry the ",
                     "line over it, or the returning treatment belongs to no ",
                     "line at all. S05 and S06 now land the same way, and the ",
                     "one day between them no longer changes the answer."),
       count_of = paste0("every return at 90 days or more - the population this rule ",
                           "moved, any drug."),
       sql = "
      WITH ep AS (
        SELECT m.PATID, m.MAP_MED_TYPE, m.MAP_END_DT,
  ...
  # S03 has the lone transplant just outside the window on a line that ends by
  # its own run-out. These three carry the rest of 3.4, on a line that is still
  # running when the transplants arrive - which is where the rule reads
  # differently from the sentence the doc used to carry.
  list(id = "S11a", group = "Transplants",
       title = "One transplant outside the window, on a line still running",
       story = paste0("A patient stays on lenalidomide and bortezomib for nearly two ",
                      "years. They have a single stem cell transplant well past the ",
                      "first 60 days, while the treatment is still going. "),
       outcome = paste0("One line, and the transplant changes nothing about it. At the ",
                        "first line the first transplant belongs to the line; outside the ",
                        "60-day window it does not hold the line open either, so the line ",
                        "ends where the drugs run out - the same day it would have with no ",
                        "transplant at all. "),
       patient = list(meds = "LEN:IMID:0:700; BORT:PI:0:700", auto = "200"),
       timeline = c("LEN   cover d0-d700",
                    "BORT  cover d0-d700",
                    "AUTO  d200  - past the 60-day window, line still running"),
       lines = c("LOT1  d0-d700  MED  DISCONTINUATION  regimen BORT LEN"),
       rule = "LOT_RULES.md 3.4",
       note = paste0("ONE line. A first transplant never ends LOT1. Outside the ",
                     "window it does not extend it either, so this line is ",
                     "identical to the same patient with no transplant.")),

  list(id = "S11b", group = "Transplants",
       title = "The SECOND transplant is the one that ends line 1",
       story = paste0("The same patient, with a second stem cell transplant 200 days ",
                      "after the first - too far apart to be a planned pair. "),
       outcome = paste0("Two lines. The first transplant belongs to line 1; the second ",
                        "ends it and opens line 2 on its own date. "),
       patient = list(meds = "LEN:IMID:0:700; BORT:PI:0:700", auto = "200,400"),
       timeline = c("LEN   cover d0-d700",
                    "BORT  cover d0-d700",
                    "AUTO  d200  - part of LOT1",
                    "AUTO  d400  - 200 days later, so not a planned pair"),
       lines = c("LOT1  d0-d399     MED       SCT_AUTO    regimen BORT LEN",
                 "LOT2  d400-d1200  SCT_AUTO  STUDY_END   regimen (none)"),
       rule = "LOT_RULES.md 3.4, 6.3",
       note = paste0("TWO lines. What ends LOT1 is the SECOND autologous ",
                     "transplant, not the first.")),

  list(id = "S11c", group = "Transplants",
       title = "With a planned tandem, it is the THIRD transplant",
       story = paste0("The same patient again, but the first two transplants are 100 ",
                      "days apart with nothing in between - a planned pair - and a third ",
                      "follows much later. "),
       outcome = paste0("Two lines. The tandem partner belongs to line 1 alongside the ",
                        "first transplant, so line 1 runs on; the third transplant is the ",
                        "one that ends it. "),
       patient = list(meds = "LEN:IMID:0:700; BORT:PI:0:700", auto = "200,300,600"),
       timeline = c("LEN   cover d0-d700",
                    "BORT  cover d0-d700",
                    "AUTO  d200  - part of LOT1",
                    "AUTO  d300  - within 180 days and nothing in between: a planned pair",
                    "AUTO  d600  - the third"),
       lines = c("LOT1  d0-d599     MED       SCT_AUTO    regimen BORT LEN",
                 "LOT2  d600-d1200  SCT_AUTO  STUDY_END   regimen (none)"),
       rule = "LOT_RULES.md 3.4, 6.3",
       note = paste0("TWO lines. A planned tandem belongs to LOT1 with the first ",
                     "transplant, so the count of transplants LOT1 absorbs is two ",
  ...
  list(id = "S30", group = "A drug that comes back",
       ...
       outcome = paste0("Two lines. The biosimilar does NOT open a third: it counts as ",
                        "the daratumumab it replaces, one drug advanced the line while ",
                        "that agent was away, so it joins the second line. Same shape ",
                        "and same answer as S01 and S02. "),
       patient = list(meds = "DARA:MAB:0:100; LEN:IMID:90:200; DARAB:MAB:250:400", subs = "DARA:DARAB"),
       timeline = c("DARA   cover d0-d100",
                    "LEN    cover d90-d200",
                    "DARAB  cover d250-d400"),
       lines = c("LOT1  d0-d89   MED  MED_ADD          regimen DARA",
                 "LOT2  d90-d400  MED  DISCONTINUATION  regimen DARAB LEN"),
       rule = "LOT_RULES.md 4.4, 4.1, 4.8",
       note = paste0("This is where the fold-in and the substitute rule meet. A ",
                     "permissible substitute is the SAME AGENT as the drug it ",
                     "replaces, so the pair shares one dose history: the interval ",
                     "counted is from the daratumumab to the biosimilar, not from the ",
                     "biosimilar's own first appearance - which it never had. One ",
                     "advance in that interval, so it folds. LOT2's regimen then ",
                     "names DARAB, the drug actually given, not the one it stands in ",
