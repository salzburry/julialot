...
LOT_DIMENSIONS <- list(
  ...
  list(id = "sct_auto_is_a_line",
       ...
       where = paste0("lot/engine/R/steps/05b_lot1_sct.R | AND datediff(ap.AUTO_DT_2, ap.AUTO_DT_1) <= {cfg$sct_tandem_days} ;; ",
                      "lot/engine/R/steps/05b_lot1_sct.R | END AS ENDING_AUTO_DT, ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | SELECT pe.PATID, min(awp.TX_DT) AS d_AUTO ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | WHEN sc.d_AUTO IS NOT NULL AND sc.d_AUTO = {least_expr} THEN 'SCT_AUTO'"),
  ...
  list(id = "sct_allo_is_a_line",
       ...
       where = paste0("lot/engine/R/steps/10_lot2_5_base.R | WHEN sc.d_ALLO IS NOT NULL AND sc.d_ALLO = {least_expr} THEN 'SCT_ALLO' ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | {if (allo_lot_span == 'single_day') 1L else 0L} = 1)"),
  ...
  list(id = "cart_is_a_line",
       ...
       where = paste0("lot/engine/R/cart_rule.R | cart_eligible_dt <- function(on, cart_col, lot1_start_col, window_days) ;; ",
                      "lot/engine/R/steps/06_lot1_end.R | AND datediff(sct.ENDING_CART_DT, date_add(lb.LOT1_BASE_1ST_ADD_MED_DT, 1)) BETWEEN 0 AND {cfg$cart_consolidation_days} ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | WHEN sc.d_CART IS NOT NULL AND sc.d_CART = {least_expr} THEN 'CART'"),
  ...
  list(id = "maintenance_is_a_line",
       ...
       where = paste0("lot/engine/R/steps/06_lot1_end.R | materialize(con, \"S16b_lot1_contains_mtx_reg\""),
  ...
  list(id = "what_starts_a_new_line",
       ...
       ours = paste0("A non-steroid agent that is not a permissible substitute ",
                     ...
                     "counts as a new agent at all. A drug of the PREVIOUS ",
                     "regimen never starts a line, whatever the gap since it ",
                     "stopped - the line it belonged to runs on over the ",
                     "return, and a drug last given further back than that is ",
                     "simply a new agent. And a drug of the immediately ",
                     "previous line's ",
       ...
       where = paste0("lot/engine/R/steps/10_lot2_5_base.R | INNER JOIN permissible_subs ps ON pma.MED_ABBR = ps.original_med ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | SELECT pe.PATID, min(ms.MAP_START_DT) AS d_MED ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | WHEN sc.d_CART IS NOT NULL AND sc.d_CART = {least_expr} THEN 'CART' ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | WHEN sc.d_MED  IS NOT NULL AND sc.d_MED  = {least_expr} THEN 'MED' ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | WHEN sc.d_AUTO IS NOT NULL AND sc.d_AUTO = {least_expr} THEN 'SCT_AUTO' ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | AND ms.MAP_MED_CLASS <> 'STEROID' ;; ",
                      "lot/engine/R/melp_rule.R | melp_suppress AS ( ;; ",
                      "lot/engine/R/melp_rule.R | melp_inject AS ( ;; ",
                      "lot/engine/R/prior_regimen.R | return_release_on <- function(cfg) !isTRUE(cfg$apply_own_return_fold) ;; ",
                      "lot/engine/R/foldin_rule.R | count(DISTINCT fo.OPENER) AS N_ADVANCES,"),
  ...
  list(id = "gap_ends_a_line",
       ...
       where = paste0("lot/engine/R/steps/03_mma_map.R | AND datediff(w.NEXT_MAP_START_DT, w.MAP_END_DT) >= {cfg$map_discon_gap_days}"),
  ...
  list(id = "regimen_membership_window",
       ...
       where = paste0("lot/engine/R/steps/10_lot2_5_base.R | AND ms.MAP_START_DT <= least( ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | date_add( ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | THEN {cart_consolidation_days - 1} ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | ELSE {induction_window_days - 1} END),"),
  ...
  list(id = "substitution",
       ...
       where = paste0("lot/engine/R/steps/10_lot2_5_base.R | INNER JOIN permissible_subs ps ON pma.MED_ABBR = ps.original_med ;; ",
                      "lot/engine/R/prior_regimen.R | INNER JOIN permissible_subs ps ON im.MED_ABBR = ps.original_med"),
  ...
  list(id = "steroids",
       ...
       ours = paste0("No. Steroids are dropped from the code list, so they ",
                     ...
                     "one alive."),
       where = paste0("lot/engine/R/steps/01_codelists.R | WHERE upper(trim(coalesce(CL_MED_CLASS, ''))) <> 'STEROID' ;; ",
                      "lot/engine/R/steps/10_lot2_5_base.R | AND ms.MAP_MED_CLASS <> 'STEROID'"),
  ...
  list(id = "line_cap",
       ...
       where = paste0("lot/engine/R/build_lot.R | coalesce(sum(CASE WHEN LOT_NUM < 1 OR LOT_NUM > {cfg$max_lot} THEN 1 ELSE 0 END), 0) AS n_bad_lot_num"),
       proves = c("max_lot"),
  ...
  list(id = "first_line_start",
       ...
       where = paste0("ndmm/R/steps/00b_lot1_index.R | WHERE bl.code IS NULL ;; ",
                      "ndmm/R/steps/00b_lot1_index.R | AND cast(t.{dt} as date) >= b.MM_DX_DT ;; ",
                      "ndmm/R/steps/00b_lot1_index.R | AND cast(t.{dt} as date) >= date('{NDMM_LOT1_FROM}') ;; ",
                      "ndmm/R/steps/00b_lot1_index.R | SELECT PATID, min(tx_dt) AS LOT1_START_DT"),
